...
test.describe('Test the API', () => {
    test('Make sure all API calls work when module is not enabled', async ({ modulePage }, testInfo) => {
        ...
        await test.step('API Method: Import Instrument-Event Mappings', async () => {
            ...
            const responseJson = await response.json();
            await expect(responseJson.error).toEqual("One or more components are missing in your input data. The following attributes are required: arm_num,unique_event_name,form");
        });
    ...
    test('Make sure all API calls work when module is enabled system-wide but not in the project', async ({ modulePage }, testInfo) => {
        ...
        await test.step('Disable module in project', async () => {
    ...
    test('All API calls work when module is enabled in the project and user has all API user rights', async ({ modulePage }, testInfo) => {
        test.setTimeout(300000);
        ...
        await test.step('Enable module in project', async () => {
            await modulePage.enableModule(config.projects.Project.pid);
        });

        await test.step('Grant all API user rights to user', async () => {
            await modulePage.grantAllApiUserRights(config.projects.Project.pid, config.users.AdminUser.username);
        });
        ...
        await test.step('API Method: Create A New Project', async () => {
            ...
            const responseJson = await response.json();
            await expect(responseJson.error).toEqual("Invalid request.");
        });
    ...
    test('All API calls are blocked when module is enabled in the project and user has no API user rights', async ({ modulePage }, testInfo) => {
        test.setTimeout(300000);
        ...
        await test.step('Grant no API user rights to user', async () => {
            await modulePage.grantNoApiUserRights(config.projects.Project.pid, config.users.AdminUser.username);
        });
        ...
        await test.step('API Method: Export Arms', async () => {
            ...
            const responseJson = await response.json();
            await expect(responseJson.error).toEqual('You do not have permissions to use this API method.');
        });
